...
void i2c_usi_init(uint8_t addr, uint8_t* txb, uint8_t txs, uint8_t* rxb, uint8_t rxs)
{
	...
	// Set USI in Two-wire mode. No USI Counter overflow prior to first Start Condition (potential failure)
...
ISR(USI_OVERFLOW_vect)
{
	...
	switch( USI_TWI_Overflow_State )
	{
		...
		case USI_SLAVE_CHECK_REPLY_FROM_SEND_DATA:
			if( d ) { // If NACK, the master does not want more data.
				SET_USI_TO_WAIT_FOR_START();
				return;
			}
